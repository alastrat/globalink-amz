...
import { ASSISTANT_NAME, IDLE_TIMEOUT, MAIN_GROUP_FOLDER, POLL_INTERVAL, TRIGGER_PATTERN, } from './config.js';
...
async function main() {
    ...
    startIpcWatcher({
        ...
        sendImage: (jid, imageUrl, caption) => {
            const channel = findChannel(channels, jid);
            if (!channel)
                throw new Error(`No channel for JID: ${jid}`);
            ...
            return channel.sendMessage(jid, caption);
        },
        sendQuotedReply: (jid, text, quotedKey) => {
            const channel = findChannel(channels, jid);
            if (!channel)
                throw new Error(`No channel for JID: ${jid}`);
            if (typeof channel.sendQuotedReply === 'function') {
                return channel.sendQuotedReply(jid, text, quotedKey);
            }
            return channel.sendMessage(jid, text);
        },
        registeredGroups: () => registeredGroups,
