...
export class WhatsAppChannel {
    ...
    async sendImage(jid, imageUrl, caption) {
        ...
        if (!this.connected) {
            logger.warn({ jid }, "WA disconnected, image message skipped");
            return null;
        }
        try {
            ...
            logger.info({ jid, imageUrl: imageUrl.substring(0, 80) }, "Image message sent");
            return sent?.key || null;
        }
        catch (err) {
            logger.warn({ jid, err }, "Failed to send image, falling back to text");
            return await this.sendMessage(jid, caption);
        }
    }

    async sendQuotedReply(jid, text, quotedKey) {
        const prefixed = ASSISTANT_HAS_OWN_NUMBER
            ? text
            : `${ASSISTANT_NAME}: ${text}`;
        if (!this.connected) {
            logger.warn({ jid }, "WA disconnected, quoted reply skipped");
            return;
        }
        try {
            ...
            logger.info({ jid, quotedId: quotedKey?.id }, "Quoted reply sent");
        }
        catch (err) {
            logger.warn({ jid, err }, "Failed to send quoted reply, falling back to regular message");
